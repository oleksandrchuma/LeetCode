...
class WordDictionary:
    def __init__(self):
        self.subTrie = defaultdict(WordDictionary)
        self.leaf = False
    ...
    def search(self, word: str) -> bool:
        if len(word) == 0:
            return self.leaf
        if word[0] == '.':
            for sub in self.subTrie.values():
                if sub.search(word[1:]):
                    return True 
            return False
        elif word[0] not in self.subTrie:
            return False
        return self.subTrie[word[0]].search(word[1:])
